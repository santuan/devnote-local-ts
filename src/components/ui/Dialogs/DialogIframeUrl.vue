...
watch(() => modal.show_iframe_url_dialog, (isOpen) => {
  if (isOpen && modal.editing_iframe_pos !== null) {
    const node = editor.value.state.doc.nodeAt(modal.editing_iframe_pos)
    if (node) {
      iframeUrl.value = node.attrs.src
    }
  }
})

function handleSubmit() {
  if (modal.editing_iframe_pos !== null) {
    editor.value.chain().focus().setNodeSelection(modal.editing_iframe_pos).updateIframe(iframeUrl.value).run()
    modal.editing_iframe_pos = null
  }
  else {
    emit('submit', iframeUrl.value)
  }
  iframeUrl.value = ''
  modal.show_iframe_url_dialog = false
}
</script>

<template>
  <DialogRoot v-model:open="modal.show_iframe_url_dialog">
    <DialogPortal>
      ...
      >
        ...
        <form class="flex flex-col gap-4" @submit.prevent="handleSubmit">
          ...
          <div class="flex justify-end gap-4">
            ...
            >
              {{ modal.editing_iframe_pos !== null ? t("verb.save") : t("verb.add") }}
            </button>
          </div>
        </form>

...
function focusDocumentsList() {
  ...
  setTimeout(() => {
    focus_store.setFocusDocumentsLists()
  }, 100)
}
...
function focusToolbar() {
  ...
  nextTick(() => {
    setTimeout(() => {
      focus_store.setFocusToolbar()
    }, 100)
  })
}

function showExportModal() {
  show_export_db.value = true
}

function showImportModal() {
  show_import_db.value = true
}
</script>
...
<template>
  <DropdownMenuRoot v-model:open="showNavigation">
    <DropdownMenuTrigger
      class="flex items-center justify-start gap-2 p-1 w-14 max-h-8 focus-visible:ring-1 focus-within:ring-1 focus-within:ring-primary focus-visible:ring-primary"
    ...
    <DropdownMenuPortal>
      <DropdownMenuContent
        side="bottom" :side-offset="5" align="start"
        class="p-1 w-78 bg-background border border-primary"
      >
        <div class="flex flex-col font-mono text-xs gap-1">
          ...
          <DropdownMenuItem
            class="text-foreground outline-hidden focus-visible:bg-primary/30 text-xs hover:bg-primary/20 px-2 py-2 text-left"
            @click="focusDocumentsList()"
          >
            {{ t("commandBar.focusDocumentsList") }}
          </DropdownMenuItem>
          <DropdownMenuItem
            class="text-foreground outline-hidden focus-visible:bg-primary/30 text-xs hover:bg-primary/20 px-2 py-2 text-left flex items-center justify-between"
            @click="focusOnTitle()"
          >
            <span>{{ t("commandBar.focusTitle") }}</span>
            <kbd
              class="pointer-events-none uppercase inline-flex h-5 select-none items-center gap-1 rounded px-1.5 pr-1 font-mono text-[10px] text-secondary-foreground font-extrabold opacity-80"
            >
              Ctrl Shit Alt <LucideArrowRight class="size-3" />
            </kbd>
          </DropdownMenuItem>
          <DropdownMenuItem
            class="text-foreground outline-hidden focus-visible:bg-primary/30 text-xs hover:bg-primary/20 px-2 py-2 text-left flex items-center justify-between"
          ...
          >
            ...
            <kbd
              class="pointer-events-none uppercase inline-flex h-5 select-none items-center gap-1 rounded px-1.5 font-mono text-[10px] text-secondary-foreground font-extrabold opacity-80"
            >
              Ctrl Shit Alt U
            </kbd>
          </DropdownMenuItem>
          <DropdownMenuItem
            ...
            @click="focusToolbar()"
          >
            <span>{{ t("commandBar.focusToolbar") }}</span>
            <kbd
              class="pointer-events-none uppercase inline-flex h-5 select-none items-center gap-1 rounded px-1.5 font-mono text-[10px] text-secondary-foreground font-extrabold opacity-80"
            >
              Ctrl Shit Alt G
          ...
          >
            <span class="text-xs">{{ t("settings.import") }}</span>
            <kbd
              class="pointer-events-none uppercase inline-flex h-5 select-none items-center gap-1 rounded px-1.5 font-mono text-[10px] text-secondary-foreground font-extrabold opacity-80"
            >
              Ctrl Shit Alt I
          ...
          <DropdownMenuItem
            class="text-foreground outline-hidden focus-visible:bg-primary/30 text-xs hover:bg-primary/20 flex items-center justify-between px-2 py-2 text-left"
            @click="showExportModal()"
          >
            <span class="text-xs">{{ t("settings.export") }}</span>
            <kbd
              class="pointer-events-none uppercase inline-flex h-5 select-none items-center gap-1 rounded px-1.5 font-mono text-[10px] text-secondary-foreground font-extrabold opacity-80"
            >
              Ctrl Shit Alt E
